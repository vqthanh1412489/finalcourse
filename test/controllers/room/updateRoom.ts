...
describe('update Room Router', () => {
    ...
    it('KT can update the Room incase idRoom && you are Admin', async () => {
        const response = await request(app).put(`/room/${idRoom}`)
        .send({ newName: 'C44', newNumberSeat: 1000 })
        .set({ token: tk });
        assert.equal(response.status, 200);
    });
    it('KT cannot delete the Room incase you arenot Admin', async () => {
